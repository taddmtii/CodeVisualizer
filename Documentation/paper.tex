...
Array searching and sorting algorithms are foundational in computer science academia. An array, by definition, is a collection of elements stored contiguously in the computer's memory. Unfortunately, students often struggle to internalize on a deeper level these concepts when taught solely through pseudocode or static diagrams.
...
Code Visualizer's effectiveness is limited to introductory algorithmic concepts, and the tool is not meant to address more advanced concepts such as graph algorithms or dynamic programming problems. The simplified Python subset Code Visualizer uses, while it is educationally beneficial for beginners, may not fully prepare students for real-world programming environments.
...
This approach has several practical advantages, but the most glaring being memory management. Memory overhead scales with the number of steps taken rather than the total number of possible execution steps, which is important given the browser-based runtime environment. It also allows the system to remain responsive during large array operations, as work is deferred until the user explicitly steps forward.
...
\centering
\scriptsize
\caption{Time and Space Complexity: Command Pattern vs.\ Snapshot-Based Approach}
\label{tab:complexity}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|l|c|c|}
\hline
\textbf{Dimension} & \textbf{Command Pattern} & \textbf{Snapshot-Based} \\ \hline
Step cost (forward/backward) & $O(k)$ & $O(1)$ \\ \hline
Initialization cost & $O(1)$ & $O(n \cdot s_{\text{total}})$ \\ \hline
Space complexity & $O(c \cdot s)$ & $O(n \cdot s_{\text{total}})$ \\ \hline
\end{tabular}
\end{table}
...
In addition to the structured lab, a small group of individuals were given open-ended access to Code Visualizer and encouraged to write any algorithm of their choosing. This informal session was intended to test the tool's more general purpose functionality and flexibility beyond the curated lab environment. The results are that the tool performed reliably across their inputs. This open ended trial in conjunction with the laboratory results suggests that Code Visualizers interpreter and visualization pipeline generalize well beyond scripted use cases.
...
There are a few known issues present in the current version of the visualizer. First, the "Link to list" option occasionally registers duplicate list entries, resulting in two identically named options that appear in the selection menu. Both entries refer to the same underlying list, so either option can be selected without affecting overall functionality. Second, the final statement or expression that is highlighted during program execution is not cleared upon completion, leaving the last line visually active after the program has finished. Third, complex expressions used as arguments within range-based loops may occasionally trigger a stack underflow error.

\subsection{Future Work}
Several directions exist for extending Code Visualizer beyond its current scope. Before all else, the outstanding bugs need to be addressed and fixed. Support for additional data structures such as linked lists or binary trees would expand the tool's applicability to a broader range of introductory courses, though this may require considerable additions to both the visualization layer and the interpreter.
